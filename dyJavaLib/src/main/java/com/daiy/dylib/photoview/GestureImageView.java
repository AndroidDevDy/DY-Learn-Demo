...
import com.daiy.dylib.tools.ScreenUtils;
...
public class GestureImageView extends ImageView implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    private static int mWidth = ScreenUtils.getDeviceWidth();
    private static int mHeight = ScreenUtils.getDeviceHeight();
    ...
    private void scale(float scale, PointF f) {
        LogTools.logi("缩放的比率：scale = " + scale);
        if (scale <= (float) 1.00) {
            scale = (float) 1.00;
        }
...
//        getImageRectF();
        ...
        LogTools.logi(TAG + ",currentScale = " + currentScale);
        setImageMatrix(mMatrix);
    }
